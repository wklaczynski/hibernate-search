/*
 * Hibernate Search, full-text search for your domain model
 *
 * License: GNU Lesser General Public License (LGPL), version 2.1 or later
 * See the lgpl.txt file in the root directory or <http://www.gnu.org/licenses/lgpl-2.1.html>.
 */
package org.hibernate.search.mapper.javabean.search.loading.spi;

import org.hibernate.search.engine.search.loading.spi.EntityLoadingStrategy;

/**
 * Contextual information about a serching type to load or index a entities during serch data.
 *
 */
public interface SearchLoadingIndexedTypeContext {

	/**
	 * @return The name of the entity in the java bean metamodel.
	 */
	String entityName();

	/**
	 * @return The loading strategy of the entity in the java bean metamodel.
	 */
	EntityLoadingStrategy loadingStrategy();

}
